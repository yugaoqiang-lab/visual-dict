  ...
  function startQuiz(){
    ...
    quizPool=list.slice().sort(dueCmp); quizScore={c:0,t:0};
  ...
  /* ---------- 生词本选择题测验（点选，移动端友好） ---------- */
  let mcqPool=[], mcqCur=null, mcqScore={c:0,t:0}, mcqCorrect='', mcqDir='en2zh';
  ...
  function launchMCQ(pool){
    mcqPool=pool.slice().sort(dueCmp); mcqScore={c:0,t:0};
    document.getElementById('list').style.display='none';
    document.querySelector('.modes').style.display='none';
  ...
  function renderStats(){
    ...
    document.getElementById('statCards').innerHTML =
      statCard('收藏', total) + statCard('已测验', tested) + statCard('已掌握', mastered, '#2e7d32') + statCard('待复习', dueN, '#f0a020') + statCard('未测验', untested, '#7a88a0');
