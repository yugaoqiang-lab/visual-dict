  ...
  function startQuiz(){
    ...
    quizPool=list.slice().sort(weakCmp); quizScore={c:0,t:0};
  ...
  function checkQuiz(){
    ...
    else { document.getElementById('qresult').innerHTML='<span style="color:#b3261e">✗ 正确是：'+quizCur[0]+' '+quizCur[1]+(quizCur[3]?' '+quizCur[3]:'')+'</span>'; recordResult(quizCur[0], false); }
    updateQScore();
  }
  ...
  function startMCQ(){
    ...
    mcqPool=pool.slice().sort(weakCmp); mcqScore={c:0,t:0};
    document.getElementById('list').style.display='none';
    document.querySelector('.modes').style.display='none';
  ...
  function checkMCQ(btn){
    ...
    else { document.getElementById('mcResult').innerHTML='<span style="color:#b3261e">✗ 正确是：'+esc(correct)+'</span>'; recordResult(mcqCur[0], false); }
    updateMcqScore();
  }
  function exitMCQ(){
  ...
  function recordResult(en, ok){ const s=stats[en]||{r:0,w:0,t:0}; if(ok) s.r++; else s.w++; s.t=Date.now(); stats[en]=s; saveStats(); }
  function weakCmp(a,b){ const sa=stats[a[0]]||{r:0,w:0}, sb=stats[b[0]]||{r:0,w:0}; return (sb.w-sb.r)-(sa.w-sa.r); }
  ...
  function renderStats(){
    const favArr=[...favs]; let tested=0, mastered=0; const rows=[];
    for(const en of favArr){
      const s=stats[en]; const r=s?(s.r||0):0, w=s?(s.w||0):0;
      if(r+w>0) tested++;
      if(r>=2 && w===0) mastered++;
      const wd=WORDS.find(z=>z[0]===en);
      rows.push({ en, r, w, weak:(w-r), ipa:wd?wd[1]:'', zh:wd?(wd[3]||''):'' });
    }
    const total=favArr.length, untested=total-tested, learning=tested-mastered;
    document.getElementById('statCards').innerHTML =
      statCard('收藏', total) + statCard('已测验', tested) + statCard('已掌握', mastered, '#2e7d32') + statCard('待巩固', learning, '#f0a020') + statCard('未测验', untested, '#7a88a0');
    rows.sort((a,b)=> b.weak - a.weak);
    const list=rows.slice(0,15);
    document.getElementById('weakList').innerHTML = list.map(x=>
      '<div class="weak-item"><span class="wk-en">'+esc(x.en)+'</span> <span class="wk-ipa">'+esc(x.ipa)+'</span> <span class="wk-zh">'+esc(x.zh)+'</span> <span class="badge">对'+x.r+'/错'+x.w+'</span></div>'
